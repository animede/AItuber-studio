...
- `app/settings.py`
  ...
  - 自己発話エージェントの既定値は `BACKGROUND_IDLE_FOLLOWUP_SECONDS=45`、`BACKGROUND_IDLE_FOLLOWUP_COOLDOWN_SECONDS=120` です。
...
- `BACKGROUND_IDLE_FOLLOWUP_SECONDS`
  - 最後のユーザー入力から、自己発話を試行するまでの待機秒数です。
  - 既定値は `45` です。
- `BACKGROUND_IDLE_FOLLOWUP_COOLDOWN_SECONDS`
  - 自己発話が実際に成功したあと、次の自己発話を抑制する秒数です。
  - 既定値は `120` です。
...
/home/animede/AItuber-studio/.venv/bin/python webapp_main.py
```
...
1. 左側の「Agent機能」を開きます。
2. 「自発話を有効にする」を ON にします。
3. 最後のユーザー入力から一定時間経つと、自動で follow-up 発話を試行します。
...
- TTS が接続されていれば、自己発話も音声つきで再生されます
